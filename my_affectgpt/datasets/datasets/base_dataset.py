...
class BaseDataset():
    def __init__(self, vis_processor=None, txt_processor=None, img_processor=None, model_cfg=None, dataset_cfg=None,
        ...
        # 【修复】拆分检查逻辑：推理时 dataset_cfg 可能为 None，但需要 llama_model_name 来确定 Prompt 格式
        if model_cfg is None: return  # 没有 model_cfg 无法继续
        ...
        self.llama_model_name = model_cfg.llama_model  # 保存模型名称用于 Prompt 模板判断
        ...
        # 【推理兼容】dataset_cfg 在推理时可能为 None，将其依赖的逻辑单独检查
        if dataset_cfg is None: return  # 推理阶段无需处理 ratio 和 debug 采样
        
        # 让模型只读取一定比例的文件（仅训练时使用）
    ...
    def func_visualize_samples(self, sample):
        ...
        print("[DEBUG text_input (问题+答案)]:")
        print(input_convert)

        label = copy.deepcopy(sample['label'])
        # 【修复】可视化时只解码有效部分，跳过 IGNORE_INDEX 位置
        # 过滤掉所有 IGNORE_INDEX (-100) 的 token，只保留有效 label
        valid_mask = label != config.IGNORE_INDEX
        valid_label = label[valid_mask]
        output_convert = self.tokenizer.decode(valid_label)
        print("[DEBUG label (仅答案部分，用于计算loss)]:")
    ...
    def _get_prompt_prefix_suffix(self):
        """
        根据模型类型返回 Prompt 的前缀、后缀和分隔符
        Gemma 3: <start_of_turn>user\n ... <end_of_turn>\n<start_of_turn>model\n ... <end_of_turn>
        其他模型: ###Human: ... ###Assistant: ... ###
        
        Returns:
            tuple: (prefix, suffix, sep) - 用户输入前缀、模型回答前缀、模型回答结束符
        """
        if hasattr(self, 'llama_model_name') and self.llama_model_name in ['Gemma3']:
            # Gemma 3 格式（注意换行符的位置）
            prefix = "<start_of_turn>user\n"
            suffix = "<end_of_turn>\n<start_of_turn>model\n"
            sep = "<end_of_turn>"  # Gemma 3 使用 <end_of_turn> 作为结束符
        else:
            # Llama/Qwen 格式
            prefix = "###Human: "
            suffix = " ###Assistant: "
            sep = "###"  # Llama/Qwen 使用 ### 作为结束符
        return prefix, suffix, sep

    def get_prompt_for_multimodal(self, face_or_frame, subtitle, user_message):
        # 获取模型特定的 Prompt 前后缀（忽略 sep，因为这里不需要）
        prefix, suffix, _ = self._get_prompt_prefix_suffix()
        ...
        if face_or_frame == 'faceframe': # (face, frame, audio, text)
            ...
            assert subtitle is not None
            prompt = f"{prefix}The subtitle of this video is: <Subtitle>{subtitle}</Subtitle>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message}{suffix}"
        elif face_or_frame == 'faceonly':  # (face)
            prompt = f"{prefix}We uniformly sample raw frames from the video and extract faces from these frames: <Video><FaceHere></Video>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message}{suffix}"
        elif face_or_frame == 'frameonly': # (frame)
            prompt = f"{prefix}We uniformly sample raw frames from the video: <Video><FrameHere></Video>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message}{suffix}"
        elif face_or_frame == 'image': # (image)
            prompt = f"{prefix}The image content is as follows: <Image><ImageHere></Image>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message}{suffix}"
        ...
        ## 后面都是增加 <Multi> token 后的结果    
            ...
            assert subtitle is not None
            prompt = f"{prefix}The audio and video merged info is: <Multi><MultiHere></Multi>. " \
                    + f"The subtitle of this video is: <Subtitle>{subtitle}</Subtitle>. " \
                    + f"Now, please answer my question based on all the provided information. {user_message}{suffix}"
        elif face_or_frame == 'multiface_audio_face_text': # (multi, face, audio, text)
            assert subtitle is not None
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                # 获取模型特定的分隔符（Gemma3: <end_of_turn>, Llama/Qwen: ###）
                _, _, sep = self._get_prompt_prefix_suffix()
                target = qa_pair['answer'] + sep
                # print (target)
    ...
    def collater(self, instances):
        ...
        # 【修复】Qwen 系列没有 bos_token_id 和 pad_token_id
        # 重要：对于训练数据，padding 值会被 attention_mask 屏蔽，不影响训练
        bos_id = self.tokenizer.bos_token_id
        pad_id = self.tokenizer.pad_token_id
        eos_id = self.tokenizer.eos_token_id
        
        # 对于 Qwen：bos_token_id 为 None，使用 eos_id 作为序列开始符（仅 1 个）
        if bos_id is None:
            bos_id = eos_id
        
        # 对于 Qwen：pad_token_id 为 None，尝试获取 <|endoftext|> 或使用 eos_id
        # 注意：padding 位置会被 attention_mask 屏蔽，所以实际值不影响训练
        if pad_id is None:
            # 尝试获取 Qwen 的 <|endoftext|> token
            try:
                pad_id = self.tokenizer.convert_tokens_to_ids('<|endoftext|>')
            except:
                pad_id = eos_id  # 回退到 eos_id
        
        for instance in instances:
            label = instance['label']
            input_id = instance['text_input']
            label    = torch.cat([torch.ones([1], dtype=input_id.dtype) * config.IGNORE_INDEX, label,
                                  torch.ones([1], dtype=input_id.dtype) * eos_id]) # (-100  xxx <eos>)
            input_id = torch.cat([torch.ones([1], dtype=input_id.dtype) * bos_id, input_id,
                                  torch.ones([1], dtype=input_id.dtype) * eos_id]) # (<bos> xxx <eos>)
            labels.append(label)
        ...
        batch = dict(
            labels=labels,
            input_ids=input_ids,
            attention_masks=input_ids.ne(pad_id),  # 使用实际的 pad_id 而不是 tokenizer.pad_token_id
        )
